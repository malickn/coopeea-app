$(function() {
    ...
    const disableMenu = (callback) => {
        const nbelems = $('.nav-menu > li').length;
        $('.nav-menu > li').each(function(index) {
            $('.nav-menu > li').find('a').removeClass('active');
            if (index + 1 === nbelems)
                callback(true);
        });
    };
        ...
        if (val) {
            ...
            $('.nav-menu li').each(function() {
                if ($(this).hasClass('active')) {
                    ...
                    const selectedDiv = $(this).parent().parent().find('a').first();
                    if (selectedDiv.text() !== 'Home' && selectedDiv.text() !== 'Accueil')
    ...
    $('.testimonials-image').slick({
        ...
        arrows: true,
        prevArrow: false,
        nextArrow: '<span class="next"> <i class="far fa-long-arrow-right"></i></span>',
    });
